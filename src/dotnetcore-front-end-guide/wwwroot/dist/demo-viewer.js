...
riot.tag2('app', '<header class="viewer-header"> <module-selector modules="{opts.modules}" default-module="{opts.defaultModule}" on-select="{setModule}"> <yield></yield> </module-selector> <button class="viewer-button {\'is-active\': infoIsOpen}" title="toggle module info" __disabled="{!activeModule.info}" onclick="{toggleInfo}"> <svg height="22" viewbox="0 0 24 24" width="22" xmlns="http://www.w3.org/2000/svg"><path d="M0 0h24v24H0z" fill="none"></path><path d="M11 17h2v-6h-2v6zm1-15C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.4 0-8-3.6-8-8s3.6-8 8-8 8 3.6 8 8-3.6 8-8 8zM11 9h2V7h-2v2z"></path></svg> </button> <size-controls on-resize="{setSize}" sizes="{opts.sizes}"></size-controls> </header> <main class="viewer-content"> <info-panel if="{infoIsOpen}" sections="{activeModule.info}" on-close="{closeInfo}"></info-panel> <iframe class="viewer-frame" frameborder="0" sandbox="allow-same-origin allow-scripts allow-forms allow-popups" riot-src="{activeModule.url}" width="{width}" height="{height}"></iframe> </main>', '', '', function (opts) {
    var tag = this;
...
riot.tag2('info-panel', '<header class="viewer-info-header"> <div class="viewer-info-control"> <button class="viewer-button" title="close info panel" onclick="{opts.onClose}"> <svg height="24" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M19 6.4L17.6 5 12 10.6 6.4 5 5 6.4l5.6 5.6L5 17.6 6.4 19l5.6-5.6 5.6 5.6 1.4-1.4-5.6-5.6z"></path><path d="M0 0h24v24H0z" fill="none"></path></svg> </button> </div> </header> <iframe if="{activeSection}" class="viewer-info-section" frameborder="0" riot-src="{sections[activeSection]}" width="100%"></iframe>', '', 'class="viewer-info" style="height:70%;"', function (opts) {
    var tag = this;
...
riot.tag2('module-selector', '<label class="viewer-header-select-wrapper"> <span class="a11y-sr-only">Module</span> <select name="selector" onchange="{onModuleSelect}"> <yield></yield> <optgroup each="{groupName, modules in moduleGroups}" label="{groupName}"> <option each="{module in modules}" value="{module.url}" __selected="{module.url == activeModule.url}"> {module.name} </option> </optgroup> </select> </label> <a href="{activeModule.url}" target="_blank" title="open module in new window" class="viewer-link"> <svg height="40" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M0 0h24v24H0z" fill="none"></path><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"></path></svg> </a>', '', 'class="viewer-module-selector"', function (opts) {
    var tag = this;
    tag.activeModule = undefined;
...
riot.tag2('size-controls', '<button each="{size in opts.sizes}" class="viewer-button {\'is-active\': size.width == width}" onclick="{onSelectSize}"> {size.name} </button> <button class="viewer-button viewer-button-full {\'is-active\': autoWidth}" onclick="{toggleAutoWidth}"> FULL </button>', '', 'class="viewer-size-controls"', function (opts) {
    var tag = this;
