...
riot.tag2('module-selector', '<label class="viewer-header-select-wrapper"> <span class="a11y-sr-only">Module</span> <select name="selector" oninput="{onModuleSelect}"> <yield></yield> <optgroup each="{groupName, modules in moduleGroups}" label="{groupName}"> <option each="{module in modules}" value="{module.url}" __selected="{module.url == activeModule.url}"> {module.name} </option> </optgroup> </select> </label> <a href="{activeModule.url}" target="_blank" title="open module in new window" class="viewer-link"> <svg height="40" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M0 0h24v24H0z" fill="none"></path><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"></path></svg> </a>', '', 'class="viewer-module-selector"', function (opts) {
    var tag = this;
    tag.activeModule = undefined;
